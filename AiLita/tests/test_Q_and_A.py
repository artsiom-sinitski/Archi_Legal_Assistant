...
def setup() -> dict[str, Any]:
    ...
    test_params["curr_date"] = datetime.now()
    try:
        ...
        sys.exit(1)
    try:
        test_params["llm_model"] = consts.AI_MODELS[test_params["model_argv"]]
        temperature: int = 0
    ...
    if q_file_name[0].endswith(".json"):
        ...
        prompt_template = prompts.sys_prompt_to_calculate_penalty_ru
    elif q_file_name[0].endswith(".docx") and "иск" in q_file_name[0]:
        test_params["q_file_type"] = "docx"
        prompt_template = prompts.sys_prompt_to_prepare_claim_ru
    else:
    ...
    # --------------------------------------------------------------------------------------------
    test_params["prompt"] = prompts.PromptTemplate(
...
def answer_law_questions(test_params: dict[str, Any], qa_chain) -> bool:
    ...
    ans_file_name: str =\
        f"Answered_{test_params["model_type"].upper()}_{test_params["topic"]}_{test_params["curr_date"].strftime('%Y%m%d')}.txt"

    with open(fr"{test_params["output_file_path"]}\{ans_file_name}", 'w', encoding="utf-8") as out_fp:
        out_fp.write(f"REPORT DATE:\t{test_params["curr_date"].strftime('%Y-%m-%d %H:%M')}\n")
        ...
        out_fp.write(f"LAW DOCS:\t{len(test_params["files"])}\n")
        out_fp.writelines([f"\t - {file}\n" for file in test_params["files"]])
...
def produce_court_claim(test_params: dict[str, Any], qa_chain) -> bool:
    return True
...
def main() -> None:
    ...
    input_tokens_rate: float = (
        consts.model_price_catalog[test_params["model_argv"]][consts.INPUT_TOKENS_PRICE_1M] / consts.ONE_MILLION
    )
    output_tokens_rate: float = (
        consts.model_price_catalog[test_params["model_argv"]][consts.OUTPUT_TOKENS_PRICE_1M] / consts.ONE_MILLION
    )
    # ------------------------------------------------------------------------------------------------------------
    ...
    qa_chain = RetrievalQA.from_chain_type(
        ...
        chain_type_kwargs={"prompt": test_params["prompt"]}
    )
    # ------------------------------------------------------------------------------------------------------------
    ...
    prompt_tokens_num = prompts.calculate_tokens_num(test_params["llm_model"], test_params["prompt"].template)
